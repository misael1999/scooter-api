...
from scooter.apps.merchants.serializers import MembersMerchantModelSerializer
# Viewset
...
class CustomerMerchantViewSet(ScooterViewSet, mixins.RetrieveModelMixin,
                              mixins.ListModelMixin, AddMerchantMixin):
    """ View set for the merchants can register a new delivery man """

    serializer_class = MembersMerchantModelSerializer
    ...
    def get_queryset(self):
        """ Personalized query when the action is a list so that it only returns active customers """
        return self.merchant.membermerchant_set.all()
