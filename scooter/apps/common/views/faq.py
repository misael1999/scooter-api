...
class FaqGroupViewSet(ScooterViewSet, mixins.ListModelMixin,
                      mixins.CreateModelMixin, mixins.UpdateModelMixin,
                      mixins.DestroyModelMixin):

    serializer_class = GroupQuestionSimpleSerializer
    pagination_class = None
    queryset = GroupQuestion.objects.filter(status=1)
    # Filters
    # ordering_fields = ('created', 'title')
